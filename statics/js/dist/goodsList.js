...
function initGrid() {
	var a = Public.setGrid(ajustH, ajustW),
		b = ["操作", "商品編號", "商品名稱", "商品類別", "規格型號", "單位", "當前庫存", "期初數量", "單位成本", "期初總價", "預計採購價", "零售價", "備註", "狀態"],
		...
		g = [{
			...
			formatter: function(a, b, c) {
				...
				var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-pencil" title="變更"></span><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pic" title="商品图片"></span></div>';
				return d
			},
			title: !1
		}, {
...
function initEvent() {
	$_matchCon = $("#matchCon"), $_matchCon.placeholder(), $("#search").on("click", function(a) {
		a.preventDefault();
		var b = "輸入商品編號、名稱、規格型號等相關資料檢索" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
		...
		if (Business.verifyRight("INVENTORY_EXPORT")) {
			var a = "輸入商品編號、名稱、規格型號等相關資料檢索" === $_matchCon.val() ? "" : $.trim($_matchCon.val()),
...
var SYSTEM = parent.SYSTEM,
	...
	thisTree, handle = {
		...
		del: function(a) {
			$.dialog.confirm("刪除的商品資料將不能恢復，請慎重操作！", function() {
