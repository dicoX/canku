    ...
    $.ajax({
      ...
      success: function(data){
        if(data.status === 200){
        	...
        	{
        		parent.Public.tips({type:2, content : '共享用戶數已經達到上限值：'+json.totalUserNum});
        		return false;
        	}else
        	{
...
<div class="wrapper">
    <div class="mod-toolbar-top">
       <a href="javascript:validMaxForShare();" class="ui-btn ui-btn-sp mrb">新增操作員</a>
       <span class="tit" id="shareInfo" style="display:none;">此賬套主服務最多支持<strong id="totalUser"></strong>操作員共同管理，已共享<strong id="usedTotal"></strong>人，剩餘<strong id="leftTotal"></strong>。</span>
    </div>    
    <div class="grid-wrap">
      <table id="grid">
      </table>
  ...
  (function($){
    ...
    $('.grid-wrap').on('click', '.delete', function(e){
      ...
      $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content: '取消授權已完成！'});
            usedTotal--;
            leftTotal++;
            showShareCount();
            if (rowData.isCom) {
            ...
            parent.Public.tips({type: 1, content: '取消授權失敗！' + data.msg});
          }
        },
        error: function(){
           parent.Public.tips({content:'取消授權失敗！請重新操作。', type: 1});
        }
      });
    });
    ...
    $('.grid-wrap').on('click', '.authorize', function(e){
      ...
      e.preventDefault();
       $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content : '授權完成！'});
            rowData.share = true;
            $("#grid").jqGrid('setRowData', id, rowData);
            usedTotal++;
            leftTotal--;
        ...
        error: function(){
          parent.Public.tips({type:1, content : '授權失敗，請重新操作。'});
        }
      });
    });
    ...
    function initGrid(){
      $('#grid').jqGrid({
        ...
        colNames:['操作員', '姓名', '公司','功能授權','數據授權','啓用授權'],
        colModel:[
...
, hidden:(parent.SYSTEM.siType == 1)},
        ...
        if (val || row.admin) {
          if (row.admin) {
              return '管理員';
               ...
               return '<div class="operating" data-id="' + row.userId + '"><span class="delete ui-label ui-label-success">已啓用</span></div>';
          }
        } else {
    ...
    function settingFormatter(val, opt, row) {
		if (row.admin || row.share === false) {
			...
			return '<div class="operating" data-id="' + row.userId + '"><a class="ui-icon ui-icon-pencil" title="詳細設置授權信息" href="../settings/authority_setting?userName=' + row.userName + '"></a></div>';
		}
    };
