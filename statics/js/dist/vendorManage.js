function init() {
	void 0 !== cRowId ? Public.ajaxPost("../basedata/contact/query?type=10&action=query", {
		...
		200 == a.status ? (rowData = a.data, initField(), initEvent(), initGrid(rowData.links)) : parent.$.dialog({
			title: "系統提示",
			content: "獲取供應商資料失敗，暫時不能編輯資料，請稍後再試！",
			icon: "alert.gif",
			max: !1,
			min: !1,
			cache: !1,
...
function initValidator() {
	$_form.validator({
		rules: {
			type: [/^[a-zA-Z0-9\-_]*$/, "編號只能由數字、字母、-或者_字符組成！"],
			unique: function(a) {
				var b = $(a).val();
				return $.ajax({
		...
		valid: function() {
			...
			Public.ajaxPost("../basedata/contact/checkname?type=10&action=checkName", {
				...
				-1 == b.status ? parent.$.dialog.confirm('供應商名稱 "' + a + '" 已經存在，是否繼續增加？', function() {
					postData()
				}, function() {}) : postData()
			})
		},
