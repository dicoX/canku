...
function initValidator() {
	$_form.validator({
		rules: {
			type: [/^[a-zA-Z0-9\-_]*$/, "編號只能由數字、字母、-或者_等字符組成"],
			unique: function(a) {
				var b = $(a).val();
				return $.ajax({
		...
		valid: function() {
			...
			Public.ajaxPost("../basedata/contact/checkName?action=checkName", {
				...
				-1 == b.status ? parent.$.dialog.confirm('客戶名稱 "' + a + '" 已經存在，是否依然繼續？', function() {
					postCustomerData()
...
function phoneCheck(a) {
	...
	return b.test(a) ? [!0, ""] : [!1, "請正確填寫手提電話號碼"]
}
function telephoneCheck(a) {
	var b = /^([\d]+-){1,2}[\d]+$/;
	return b.test(a) ? [!0, ""] : [!1, "請正確填寫座機號碼，格式為853-1234567或086-0754-1234567。"]
}
