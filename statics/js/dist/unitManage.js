function initField() {
	if (groupCombo = $("#group").combo({
		...
		callback: {
			onChange: function(a) {
				if (a) {
					for (var b = defaultPage.SYSTEM.unitInfo, c = !0, d = 0; d < b.length; d++) a.id == b[d].unitTypeId && (c = !0);
...
function initUnit() {
	if ($.isEmptyObject(rowData)){
		var data = defaultPage.SYSTEM.unitInfo;
		var str = '';
		for (var i in data){
			str += data[i].name+':<input class="unit" data-id="'+data[i].id+'"><br/>';
		}
		$('.rate').parents('.row-item').show();
		$('.rate').html(str);
	}else{
		Public.ajaxGet("../basedata/unit/unitRate",{
			id:rowData.id
		},function (data) {
			if (data.status==200) {
				var data = data.data;
...
initPopBtns(), initField(), initEvent(), isPostback = !0, 1 === siType && $(".row-item:gt(0)").hide();initUnit();