...
function(a) {
			...
			success: function(a) {
				if (200 == a.status) {
					...
					if (b.shareTotal >= b.totalUserNum) return Public.tips({
						type: 2,
						content: "共享管理員人數已經到達上限值：" + b.totalUserNum
					}), !1
				}
			}
		})
	...
	function c() {
		var b = a.trim(a("#userName").val()),
			c = a('<span class="loading"><i class="ui-incon ui-icon-loading"></i>檢查賬戶名...</span>').insertAfter(a("#userName"));
		a("#registerForm").data("onPost", !0), a.ajax({
			url: "../right/queryUserByName?userName=" + b,
			dataType: "json",
			type: "POST",
			success: function(b) {
				c.remove(), a("#registerForm").data("onPost", !1), 200 == b.status ? (a("#userName").data("valid", !1), h(a("#userName"), !1, "此賬戶名已經存在")) : (a("#userName").data("valid", !0), h(a("#userName"), !0))
			}
		})
	}
	function d() {
	...
	var j = {
		...
		k = {
			...
			password: {
				...
				max: "密鑰長度限定是6-20位！",
				notAllNum: "密鑰不能全爲數字！",
				password: "密鑰必須由英文字母（區分大小寫）、數字或者特殊符號組合！"
			},
			pswConfirm: {
				required: "請再次輸入密鑰！",
				equalTo: "兩次輸入的密鑰不相同！"
...
//			mobile: /^(13|15|18)[0-9]{9}$/,
	...
	a("#registerForm input").on("focus", function() {
		...
		var d = "密鑰強度";
		1 == c ? d += "：弱" : 2 == c ? d += "：中等" : 3 == c && (d += "：強"), a("#pswStrength p").text(d)
	})
}(jQuery);