		...
		addEvent: function() {
			...
			$(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("QTZC_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("確定要刪除此項支出記錄嗎？", function() {
				...
				queryConditions.matchCon = "請輸入單據編號/客戶名稱/備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), THISPAGE.reloadData(queryConditions)
