...
function initEvent() {
	$("#btn-add").click(function(a) {
		...
		conditions.skey = "輸入類別名稱" == b ? "" : b, $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid")
	...
	$("#grid").jqGrid({
		...
		loadComplete: function(a) {
			if (a && 200 == a.status) {
				...
				var f = 250 == a.status ? "沒有" + conditions.name + "分類數據！" : "獲取" + conditions.name + "分類數據失敗！" + a.msg;
				parent.Public.tips({
					type: 2,
					content: f
				})
...
function postData(a) {
	...
	var f = {
		...
		g = "add" == d ? "新增" + conditions.name + "分類" : "編輯" + conditions.name + "分類";
...
var conditions = {
	...
	handle = {
		operate: function(a, b) {
			if ("add" == a) {
				var c = "新增" + conditions.name + "分類";
				({
					oper: a,
					callback: this.callback
				})
			} else {
				var c = "編輯" + conditions.name + "分類";
				({
					oper: a,
			...
			var d = ['<form id="manage-form" action="">', '<ul class="mod-form-rows manage-wrap" id="manager">', '<li class="row-item" style="position:relative; display:none;">', '<div class="label-wrap"><label for="ParentCategory">上層分類：</label></div>', '<div class="ctn-wrap" style="position:relative;"><input type="text" value="" class="ui-input" name="ParentCategory" id="ParentCategory" readonly></div>', '<div class="dn hideFeild"></div>', "</li>", '<li class="row-item">', '<div class="label-wrap"><label for="category">分類名：</label></div>', '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="category" id="category"></div>', "</li>", "</ul>", "</form>"],
				e = 90;
		...
		del: function(a) {
			$.dialog.confirm("刪除的" + conditions.name + "分類將不能恢復，請慎重操作！", function() {
				Public.ajaxPost("../basedata/assist/delete?action=delete", {
					...
					if (b && 200 == b.status) {
						parent.Public.tips({
							content: "刪除" + conditions.name + "分類完成！"
						...
						type: 1,
						content: "刪除" + conditions.name + "分類失敗！" + b.msg
					})
				})
			})
		},
