...
<div class="wrapper">
  <div class="mod-toolbar-top cf">
    <div class="fl"><h3 class="f14">管理權限設定<span class="fwn">（請勾選爲 <b id="userName"></b> 分配的權限）</span></h3></div>
...
<script>
  ...
  var relation = {
			"入貨單":[{name:'商品',rights:['查詢']},{name:'庫存',rights:['查詢']},{name:'供應商',rights:['查詢']}],
			"銷售單":[{name:'商品',rights:['查詢']},{name:'庫存',rights:['查詢']},{name:'客戶',rights:['查詢']}],
			"調撥單":[{name:'商品',rights:['查詢']},{name:'庫存',rights:['查詢']}]
	},
	$grid = $('#grid'),
  ...
  $("#grid").jqGrid({
	  ...
	  //gridview: true,
	  colNames:['<input type="checkbox" id="all" class="vm">', '功能模組', '操作權限', '<label for="all">授權</label>'],
	  colModel:[
  ...
  $('#save').click(function(e){
	  ...
	  Public.ajaxPost('../right/addRights2OutUser?userName=' + userName + '&rightid='+ items.join(','), {}, function(data){
		  if(data.status === 200) {
			  parent.Public.tips({content : '儲存完成！'});
		  } else {
