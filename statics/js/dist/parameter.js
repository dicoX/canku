		...
		addEvent: function() {
			...
			$("#save").click(function() {
				if (Business.verifyRight("SYSTEM_UPDATE")) {
					...
					$.dialog.confirm("變更系統參數將會刷新所有頁面，是否繼續進行？", function() {
						var a = parent.$.dialog.tips("提交中，請稍等...", 1e3, "loading.gif", !0);
						Public.ajaxPost("../basedata/systemprofile/update", g, function(b) {
							200 === b.status ? (parent.window.$.cookie("ReloadTips", "系統參數設置完成"), parent.window.location.reload()) : (parent.Public.tips({
								type: 1,
								content: b.msg
							}), a.close())
						}, function() {
