var queryConditions = {
	...
	THISPAGE = {
		...
		loadGrid: function() {
			function a(a, b, c) {
				var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-pencil" title="編輯"></span><span class="ui-icon ui-icon-trash" title="刪除"></span></div>';
				return d
			}
			function b(a) {
				var b;
		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("IO_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("確定要刪除此入庫記錄嗎？", function() {
			...
			$("#search").click(function() {
				queryConditions.matchCon = "請輸入單據編號/供應商名稱/備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.locationId = -1, queryConditions.transTypeId = -1, THISPAGE.reloadData(queryConditions)
			}), $("#moreCon").click(function() {
				queryConditions.matchCon = "請輸入單據編號/供應商名稱/備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), $.dialog({
					id: "moreCon",
					width: 480,
					height: 330,
					min: !1,
