		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("OO_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("確定要刪除此項出貨記錄嗎？", function() {
			...
			$("#search").click(function() {
				queryConditions.matchCon = "請輸入單據編號/客戶名稱/備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.locationId = -1, queryConditions.transTypeId = -1, THISPAGE.reloadData(queryConditions)
			}), $("#moreCon").click(function() {
				queryConditions.matchCon = "請輸入單據編號/客戶名稱/備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), $.dialog({
					id: "moreCon",
					width: 480,
					height: 330,
					min: !1,
