    ...
    THISPAGE = {
        ...
        initDom: function (a) {
            ...
            if (this.$_customer = $("#customer"), this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_checkName = $("#checkName"), this.$_customerFree = $("#customerFree"), this.customerArrears = 0, this.$_note.placeholder(), "150602" == originalData.transType) {
                {
                    parent.$("#page-tab").find("li.l-selected").children("a").html("銷售退貨單")
                }
                $("#paymentTxt").html("是次退款：")
            } else {
                {
                    parent.$("#page-tab").find("li.l-selected").children("a").html("出貨單")
                }
                $("#paymentTxt").html("是次實收：")
            }
            ...
            var e = '<!--<a id="original" class="ui-btn ui-btn-sc">選取源單</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">儲存並增加</a><a id="save" class="ui-btn">儲存</a>',
                f = '<!--<a id="original" class="ui-btn ui-btn-sc">選取源單</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a><a id="edit" class="ui-btn">儲存</a>',
                g = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a>',
                h = "",
                i = "",
                j = "";
            billRequiredCheck ? (h = '<a class="ui-btn" id="audit">審覈</a>', i = '<a class="ui-btn" id="reAudit">反審覈</a>') : this.$_checkName.parent().hide();
            var k = '<a class="ui-btn-prev" id="prev" title="前一張"><b></b></a><a class="ui-btn-next" id="next" title="后一張"><b></b></a>';
        ...
        loadGrid: function (a) {
            ...
            var y = [{
                ...
                name: "goods",
                label: "商品",
                nameExt: '<span id="barCodeInsert">掃描槍錄入</span>',
                width: 300,
                classes: "goods",
                formatter: c,
                editable: !0,
            ...
            this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
                ...
                callback: {
                    onChange: function (data) {
                        ...
                        function p(price) {
                            var d = price,
                                l = parseFloat($("#grid").jqGrid("getCell", THISPAGE.curID, 14 - 1)),
                                h = parseFloat($("#grid").jqGrid("getCell", THISPAGE.curID, 14 + 1));
                            if ($.isNumeric(l)) if ($.isNumeric(h)) var i = d * l * h / 100,
                                j = d * l - i,
                                k = $("#grid").jqGrid("setRowData", THISPAGE.curID, {
                                    price: d,
                                    mainUnit: data.name,
                                    deduction: i,
                                    amount: j
                                });
                            else var k = $("#grid").jqGrid("setRowData", THISPAGE.curID, {
                                    price: d,
                                    mainUnit: data.name,
                                    amount: d * l
                                });
                            b(THISPAGE.curID), k && THISPAGE.calTotal();
                        }
                        ...
                        if (data) {
                            ...
                            var price;
                            var contact = THISPAGE.$_customer.data("contactInfo");
                            if (unit_id != to_unit_id) {
                                Public.ajaxPost("../scm/invSa/unit?action=unit", {
                                    unit_id: unit_id,
        ...
        addEvent: function () {
            ...
            this.$_date.bind("keydown", function (a) {
                ...
                b.test($(this).val()) || (parent.Public.tips({
                    type: 2,
                    content: "日期格式錯誤，正確示例：2012-08-08。"
                ...
                if (a.preventDefault(), Business.verifyRight("SA_ADD")) {
                    var b = "出貨單",
                        c = "sales-sales";
                    if ("150602" == originalData.transType) var b = "銷售退貨單",
                        c = "sales-salesBack";
                ...
                return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
                    type: 2,
                    content: "已經沒有前一張！"
                }), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("數據加載中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/update?action=update", {
                ...
                return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
                    type: 2,
                    content: "已經沒有下一張！"
                }), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("數據加載中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/update?action=update", {
                    id: a.idList[a.idPostion]
                }, function (a) {
            ...
            if (f.length > 0) {
                var g = $.trim(b.$_note.val()),
                    h = {
                        ...
                        totalArrears: $.trim(Number(b.$_totalArrears.val()) ? Number(b.$_totalArrears.val()) : "0.00"),
                ...
                if (taxRequiredCheck && (h.totalTax = $("#grid").jqGrid("footerData", "get").tax.replace(/,/g, ""), h.totalTaxAmount = $("#grid").jqGrid("footerData", "get").taxAmount.replace(/,/g, "")), requiredMoney) {
                    ...
                    if (0 !== Number(h.rpAmount) && 0 === h.accId) return parent.Public.tips({
                        type: 1,
                        content: i + "不爲零時，請選取結算賬戶！"
                    }), !1;
                    if (0 === Number(h.rpAmount) && 0 !== h.accId) return parent.Public.tips({
                        type: 1,
                        content: i + "不零時，不要選取結算賬戶！"// "結算賬戶不能爲空，" + i + "！"
                    }), !1;
